...
export const TOOLS: Anthropic.Tool[] = [
  ...
  {
    name: "cron_create",
    description: "Schedule a recurring or one-shot task with a cron expression. The prompt will be injected into the conversation when the schedule fires.",
    input_schema: {
      type: "object" as const,
      properties: {
        cron: {
          type: "string",
          description: "5-field cron expression: min hour dom month dow. Ex: */5 * * * * (every 5min), 0 9 * * 1 (Mon 9am), 30 14 * * * (daily 2:30pm)",
        },
        prompt: {
          type: "string",
